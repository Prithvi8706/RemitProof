"use client";

import { AlertCircle, RotateCcw } from "lucide-react";
import { AppHeader } from "@/components/AppHeader";

export default function BenchmarkError({ reset }: { error: Error & { digest?: string }; reset: () => void }) {
  return (
    <div className="case-site min-h-screen bg-canvas">
      <AppHeader />
      <main className="mx-auto max-w-[760px] px-4 py-20 sm:px-6">
        <div className="border border-danger/25 bg-danger-soft p-7 sm:p-9">
          <AlertCircle className="size-7 text-danger" aria-hidden="true" />
          <h1 className="mt-5 text-2xl font-semibold tracking-[-0.025em] text-ink">Benchmark is unavailable</h1>
          <p className="mt-3 max-w-[60ch] text-sm leading-6 text-muted">
            The published result artifacts could not be loaded or failed integrity validation. Confirm that the API
            service is available and that a result generation has been published, then try again.
          </p>
          <button
            type="button"
            onClick={reset}
            className="mt-6 inline-flex items-center gap-2 rounded-[10px] bg-primary px-4 py-2.5 text-sm font-semibold text-white hover:bg-primary-dark"
          >
            <RotateCcw className="size-4" aria-hidden="true" />
            Retry benchmark
          </button>
        </div>
      </main>
    </div>
  );
}

...
export default function ExceptionQueueError({ reset }: { error: Error & { digest?: string }; reset: () => void }) {
  return (
    <div className="min-h-screen bg-canvas">
      ...
      <main className="mx-auto max-w-[760px] px-4 py-20 sm:px-6">
        <div className="border border-danger/25 bg-danger-soft p-7 sm:p-9">
          ...
          <h1 className="mt-5 text-2xl font-semibold tracking-[-0.025em] text-ink">Exception queue is unavailable</h1>
          <p className="mt-3 max-w-[60ch] text-sm leading-6 text-muted">
            The investigation list could not be loaded. Confirm that the API service is available, then try again.
          </p>
          <button
            type="button"
            onClick={reset}

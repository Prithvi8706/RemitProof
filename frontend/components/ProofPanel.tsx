...
export function ProofPanel({ detail }: { detail: ExceptionDetail }) {
  ...
  const authorized = Boolean(detail.resolution_proof);
  const rows: CheckRow[] = [
    {
      label: "Arithmetic",
  ...
  return (
    <section aria-labelledby="proof-title">
      <div className="mb-4 flex items-center justify-between gap-4">
        <div>
          <h2 id="proof-title" className="text-xl font-semibold tracking-[-0.02em] text-ink">{authorized ? "Resolution proof" : "Financial proof"}</h2>
          <p className="mt-1 text-sm text-muted">{authorized ? "The authorization certificate generated by deterministic code." : "The deterministic checks that prevented authorization."}</p>
        </div>
        {sufficiency && (
      ...
      <div className={`mb-5 border px-4 py-3 text-sm leading-6 ${
        sufficiency?.safe_to_resolve
          ? "border-primary/25 bg-primary-soft text-primary-dark"
          : "border-warning/30 bg-warning-soft text-ink"
      }`}>
        {sufficiency?.safe_to_resolve
          ? "Proof complete: every required check passed and the evidence supports one unique allocation."
          : "Proof incomplete: no authorization was issued. The failed or unresolved checks below explain the abstention."}
      </div>
